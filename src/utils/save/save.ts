...
export const commonSave = async (state: State) => {
    ...
    console.log("Voici la value à save: ", state.formValues[state.stepConfig.fieldName])
    ...
    const { errors:answerErrors} = await client.mutate<AnswerMutation>({
      mutation: Answer,
      variables: { values: answer, session: state.sessionId, field: state.stepConfig.fieldName, order: state.visitedSteps.length, question: state.currStep!.id },
    });
